...
Let's launch the work chain with inputs that will cause the calculation to fail, e.g. by making on of the operands negative, and see what happens:
...
However, since the inputs were not changed, the calculation inevitably and wholly unexpectedly failed once more with the exact same error code.
